...
class RandomRotation(object):
    def __call__(self, image, masks, boxes, labels):
        ...
        if random.randint(2):
            ...
            rand_data = random.randint(1,18)*5

            image = rotate(image, rand_data , reshape=False)
            masks = np.array([rotate(mask, rand_data , reshape=False) for mask in masks])
...
class PrepareMasks(object):
    ...
    def __call__(self, image, masks, boxes, labels=None):
        ...
        return image, new_masks, boxes, label
