...
class RandomRotation(object):
    def __call__(self, image, masks, boxes, labels):
        ...
        if random.randint(2):
            ...
            image = rotate(image, random.randint(0,360), reshape=False)
            masks = np.array([rotate(mask, random.randint(0,360), reshape=False) for mask in masks])

            boxes = boxes.copy()
        
            boxes = np.zeros(boxes.shape, dtype=np.float32)
